...
static int callback_message(int event, void *event_data, void *userdata)
{
	...
	//long long unix_ms = ((long long)ts.tv_sec * 1000) + (ts.tv_nsec / 1000000);
	long long unix_ns = ((long long)ts.tv_sec * 1000000000LL) + ts.tv_nsec;

	char timestamp_str[32];

	snprintf(timestamp_str, sizeof(timestamp_str), "%lld", unix_ns);


	UNUSED(event);
	UNUSED(userdata);

	return mosquitto_property_add_string_pair(&ed->properties, MQTT_PROP_USER_PROPERTY, "timestamp", timestamp_str);
}
